...
const GeomorphologyMap = ({ gpId, title, isPrint, projectCenter }) => {
    ...
    const mapId = useMemo(() => `geomorph-map-${gpId}-${Math.random().toString(36).substr(2, 5)}`, [gpId]);
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                const data = await locationService.getGeomorphologyMap(gpId, lat, lon);
                ...
                setMapUrl(`${BASE_URL}/geomorphology-map/?gp_id=${gpId}${lat ? `&lat=${lat}&lon=${lon}` : ''}`);
                setLegendUnits(data.found_units || []);
            } catch (err) {
                console.error("Error fetching geomorphology map:", err);
            } finally {
                setLoading(false);
            }
        };
    ...
    if (loading || !boundary || !bounds) {
        return (
            <PlaceholderImage
                title={title}
                height="400px"
                message={loading ? "Exploring Geomorphology..." : "Initializing..."}
            />
        );
    }
            ...
            >
                ...
                <div style={{ flex: '1', position: 'relative', overflow: 'hidden', height: '100%', backgroundColor: '#f8fafc', borderRight: '1px solid #e2e8f0' }}>
                    <svg width="100%" height="100%" viewBox={`0 0 ${svgW} ${svgH}`} preserveAspectRatio="xMidYMid meet">
                        ...
                        {/* Boundary and Markers (Not Clipped) */}
                        {!boundary.is_buffer ? (
                    ...
                    <div style={{ flex: 1, overflowY: 'auto' }}>
                        {legendUnits.map((unit, idx) => (
                            <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                                ...
                                <span style={{ fontSize: '0.65rem', fontWeight: 600, color: '#475569' }}>{unit.name}</span>
                            </div>
                        ))}
                    </div>
                </div>
