...
import { RECHARGE_TYPES, WORK_PROPOSED_OPTIONS } from './supplyConfig';

const RechargeStructure = ({ data, onAdd, onDelete }) => {
    ...
    const annualGWRecharge = (parseFloat(inputs.storageCapacity || 0) * parseFloat(inputs.annualFillings || 0) * (parseFloat(inputs.rechargePercent || 0) / 100)).toFixed(4);

    const handleAdd = () => {
        if (!inputs.villageName || !inputs.structureType) return;
        onAdd({ ...inputs, annualGWRecharge });
        setInputs({
            villageName: '', locationDetails: '', structureType: '', latitude: '', longitude: '',
            workProposed: '', storageCapacity: '', annualFillings: '', rechargePercent: '',
    ...
    return (
        <div className="supply-section-content">
            <h3 className="section-title">Artificial Recharge Structures</h3>
            ...
            <div className="entry-form-container">
                <div className="form-grid-vertical">
                    ...
                    <div className="form-item">
                        <label>Type of Artificial recharge Structure* :</label>
                        <select value={inputs.structureType} onChange={e => setInputs({ ...inputs, structureType: e.target.value })}>
                            <option value="">--Select Structure--</option>
                            {RECHARGE_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                        </select>
                    </div>
                    <div className="form-item">
                    ...
                    <div className="form-item">
                        ...
                        <input type="number" value={inputs.rechargePercent} onChange={e => setInputs({ ...inputs, rechargePercent: e.target.value })} />
                    </div>
                    <div className="form-item">
                        <label>Annual GW Recharge (ha m) :</label>
                        <input type="text" value={annualGWRecharge} readOnly className="readonly-input" />
                    </div>
                    <div className="form-item">
                ...
                <table className="demand-table">
                    <thead>
                        <tr>
                            ...
                            <th>Type of Artificial recharge Structure</th>
                            <th>Latitude</th>
                            <th>Longitude</th>
                            <th>Work proposed</th>
                            ...
                            <th>Recharge (%)</th>
                            <th>Annual GW Recharge (ha m)</th>
                            <th>Financial Year</th>
                            <th>Department Name</th>
                            <th>Scheme Name</th>
                            <th>Total Approximate Estimated Cost(in lakh)</th>
                            <th>Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        {data.map((item, index) => (
                            <tr key={item.id}>
                                ...
                                <td>{item.rechargePercent}</td>
                                <td className="highlight-val">{item.annualGWRecharge}</td>
